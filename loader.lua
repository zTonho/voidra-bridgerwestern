...
local State = {
    ...
    Environment = {
        ...
        WaterConnection = nil,
        LavaConnection = nil,
        LavaDescendantConnection = nil,
        WaterOriginalDisabled = {},
        WaterOriginalDivingGearSet = false,
        WaterOriginalDivingGear = nil,
        LavaOriginalProperties = {},
        LavaLastApply = 0,
        ForceOreRender = false,
        RenderConnection = nil,
        HiddenObjectsConnection = nil,
        OriginalOptimizationSet = false,
        OriginalOptimization = nil,
        OriginalRenderDistanceSet = false,
        OriginalRenderDistance = nil,
        LastRenderApply = 0,
    },
    Mining = {
        SelectedOre = "Copper",
...
local EnvironmentState = State.Environment
...
local function getWaterHandlerScripts()
    local scripts = {}
    local containers = {
        ...
        if container then
            for _, object in ipairs(container:GetDescendants()) do
                if object:IsA("LocalScript") or object:IsA("ModuleScript") then
                    ...
                    if object:FindFirstChild("WaterIncrement")
                        or object:FindFirstChild("WaterAmbience")
                        or object:FindFirstChild("_WaterBlur")
                        or (lowerName:find("water", 1, true) and object:FindFirstChild("Tile"))
                    then
...
local function clearWaterClientState()
    pcall(function()
        RunService:UnbindFromRenderStep("WaterAmbience")
    end)

    LocalPlayer:SetAttribute("Underwater", nil)
    LocalPlayer:SetAttribute("WaterColor", nil)

    local character = getMainCharacter()

    if character then
        if not EnvironmentState.WaterOriginalDivingGearSet then
            EnvironmentState.WaterOriginalDivingGear = character:GetAttribute("DivingGearEquipped")
            EnvironmentState.WaterOriginalDivingGearSet = true
        end

        character:SetAttribute("DivingGearEquipped", true)
        character:SetAttribute("Underwater", nil)
    end

    local blur = Lighting:FindFirstChild("_WaterBlur")
    local color = Lighting:FindFirstChild("_WaterColor")
    ...
    local underwater = sfx and sfx:FindFirstChild("Underwater")

    if underwater then
        underwater.Enabled = false
    end

    local values = LocalPlayer:FindFirstChild("Values")

    if values then
        for _, value in ipairs(values:GetDescendants()) do
            local lowerName = value.Name:lower()

            if lowerName:find("oxygen", 1, true)
                or lowerName:find("breath", 1, true)
                or lowerName:find("air", 1, true)
            then
                if value:IsA("NumberValue") or value:IsA("IntValue") then
                    value.Value = math.max(value.Value, 100)
...
local function setWaterProtectionEnabled(enabled)
    EnvironmentState.NoDrown = enabled

    if EnvironmentState.WaterConnection then
        EnvironmentState.WaterConnection:Disconnect()
        EnvironmentState.WaterConnection = nil
    end

    if enabled then
        for _, scriptInstance in ipairs(getWaterHandlerScripts()) do
            if EnvironmentState.WaterOriginalDisabled[scriptInstance] == nil then
                EnvironmentState.WaterOriginalDisabled[scriptInstance] = scriptInstance:GetAttribute("Disabled") == true
            end

            scriptInstance:SetAttribute("Disabled", true)
        end

        clearWaterClientState()

        EnvironmentState.WaterConnection = RunService.Heartbeat:Connect(function()
            for _, scriptInstance in ipairs(getWaterHandlerScripts()) do
                scriptInstance:SetAttribute("Disabled", true)
            end
        ...
        mainNotify("Water damage disabled.")
        return
    end

    for scriptInstance, wasDisabled in pairs(EnvironmentState.WaterOriginalDisabled) do
        if scriptInstance and scriptInstance.Parent then
            scriptInstance:SetAttribute("Disabled", wasDisabled or nil)
        end

        EnvironmentState.WaterOriginalDisabled[scriptInstance] = nil
    end

    local character = getMainCharacter()

    if character and EnvironmentState.WaterOriginalDivingGearSet then
        character:SetAttribute("DivingGearEquipped", EnvironmentState.WaterOriginalDivingGear)
    end

    EnvironmentState.WaterOriginalDivingGearSet = false
    EnvironmentState.WaterOriginalDivingGear = nil
    mainNotify("Water damage restored.")
end

local function isLavaKillPart(part)
    if not part or not part:IsA("BasePart") then
        return false
    end

    return part.Name == "KillPart" and instanceTreeHasNameHint(part, { "lava", "magma" })
end

local function setLavaPartDisabled(part, disabled)
    if not part or not part.Parent then
        return
    end

    if disabled then
        if not EnvironmentState.LavaOriginalProperties[part] then
            EnvironmentState.LavaOriginalProperties[part] = {
    ...
    local original = EnvironmentState.LavaOriginalProperties[part]

    if original then
        part.CanTouch = original.CanTouch
        part.CanCollide = original.CanCollide
        part.CanQuery = original.CanQuery
        EnvironmentState.LavaOriginalProperties[part] = nil
    end
end

local function applyLavaProtection()
    local map = workspace:FindFirstChild("Map")
    local rosewell = map and map:FindFirstChild("Rosewell")
    local magmaCave = rosewell and rosewell:FindFirstChild("MagmaCave")
    local searchRoot = magmaCave and magmaCave:FindFirstChild("Lava")
        or (map and map:FindFirstChild("Lava", true))
        or workspace:FindFirstChild("Lava", true)

    if searchRoot then
        for _, object in ipairs(searchRoot:GetDescendants()) do
            if isLavaKillPart(object) then
                setLavaPartDisabled(object, true)
            end
        end
    end
end

local function setLavaProtectionEnabled(enabled)
    EnvironmentState.NoLava = enabled

    if EnvironmentState.LavaConnection then
        EnvironmentState.LavaConnection:Disconnect()
        EnvironmentState.LavaConnection = nil
    end

    if EnvironmentState.LavaDescendantConnection then
        EnvironmentState.LavaDescendantConnection:Disconnect()
        EnvironmentState.LavaDescendantConnection = nil
    end

    if enabled then
        applyLavaProtection()

        EnvironmentState.LavaConnection = RunService.Heartbeat:Connect(function()
            local now = os.clock()

            if now - EnvironmentState.LavaLastApply >= 0.5 then
                EnvironmentState.LavaLastApply = now
                applyLavaProtection()
            end
        end)

        EnvironmentState.LavaDescendantConnection = workspace.DescendantAdded:Connect(function(object)
            if isLavaKillPart(object) then
                setLavaPartDisabled(object, true)
            end
        end)

        mainNotify("Lava touch disabled.")
        return
    end

    for part in pairs(EnvironmentState.LavaOriginalProperties) do
        setLavaPartDisabled(part, false)
    end

    mainNotify("Lava touch restored.")
end

local function getLocalValue(name)
    local values = LocalPlayer:FindFirstChild("Values")
    return values and values:FindFirstChild(name)
end

local function getHiddenObjectsFolder()
    return ReplicatedFirst:FindFirstChild("HiddenObjects")
end

local function getWorldSpawnFolder(name)
    local worldSpawn = workspace:FindFirstChild("WorldSpawn")
    return worldSpawn and worldSpawn:FindFirstChild(name)
end

local function isWorldSpawnRenderableObject(object)
    return object
        and object:IsA("Model")
        and (object.PrimaryPart or object:FindFirstChildWhichIsA("BasePart", true))
end

local function restoreHiddenWorldObject(object)
    if not isWorldSpawnRenderableObject(object) then
        return false
    end

    local targetFolder = object:GetAttribute("ChopColor") and getWorldSpawnFolder("Trees") or getWorldSpawnFolder("Ores")

    if not targetFolder then
        return false
    end

    object.Parent = targetFolder
    return true
end

local function applyRenderBoost()
    local optimization = getLocalValue("Optimization")
    local renderDistance = getLocalValue("RenderDistance")

    if optimization and optimization:IsA("BoolValue") then
        if not EnvironmentState.OriginalOptimizationSet then
            EnvironmentState.OriginalOptimization = optimization.Value
            EnvironmentState.OriginalOptimizationSet = true
        end

        optimization.Value = false
    end

    if renderDistance and (renderDistance:IsA("NumberValue") or renderDistance:IsA("IntValue")) then
        if not EnvironmentState.OriginalRenderDistanceSet then
            EnvironmentState.OriginalRenderDistance = renderDistance.Value
            EnvironmentState.OriginalRenderDistanceSet = true
        end

        renderDistance.Value = math.max(renderDistance.Value, 12)
    end

    local hiddenObjects = getHiddenObjectsFolder()
    local restored = 0

    if hiddenObjects then
        for _, object in ipairs(hiddenObjects:GetChildren()) do
            if restoreHiddenWorldObject(object) then
                restored = restored + 1
            end
        end
    end
...
local function setRenderBoostEnabled(enabled)
    EnvironmentState.ForceOreRender = enabled

    if EnvironmentState.RenderConnection then
        EnvironmentState.RenderConnection:Disconnect()
        EnvironmentState.RenderConnection = nil
    end

    if EnvironmentState.HiddenObjectsConnection then
        EnvironmentState.HiddenObjectsConnection:Disconnect()
        EnvironmentState.HiddenObjectsConnection = nil
    end

    if enabled then
        applyRenderBoost()

        EnvironmentState.RenderConnection = RunService.Heartbeat:Connect(function()
            local now = os.clock()

            if now - EnvironmentState.LastRenderApply >= 1 then
                EnvironmentState.LastRenderApply = now
                applyRenderBoost()
            end
        end)
        ...
        if hiddenObjects then
            EnvironmentState.HiddenObjectsConnection = hiddenObjects.ChildAdded:Connect(function(object)
                task.defer(function()
                    if EnvironmentState.ForceOreRender then
                        restoreHiddenWorldObject(object)
                    end
                end)
            end)
        end

        mainNotify("Ore rendering forced.")
        return
    end
    ...
    if optimization and optimization:IsA("BoolValue") and EnvironmentState.OriginalOptimizationSet then
        optimization.Value = EnvironmentState.OriginalOptimization == true
    end

    if renderDistance
        and (renderDistance:IsA("NumberValue") or renderDistance:IsA("IntValue"))
        and EnvironmentState.OriginalRenderDistanceSet
    then
        renderDistance.Value = EnvironmentState.OriginalRenderDistance
    end

    EnvironmentState.OriginalOptimizationSet = false
    EnvironmentState.OriginalOptimization = nil
    EnvironmentState.OriginalRenderDistanceSet = false
    EnvironmentState.OriginalRenderDistance = nil
    mainNotify("Ore rendering restored.")
end

local SurvivalBox = Tabs.Main:AddLeftGroupbox("Survival", "shield")

SurvivalBox:AddToggle("SurvivalNoDrown", {
    Text = "No drown",
    Default = false,
})

SurvivalBox:AddToggle("SurvivalNoLava", {
    Text = "No lava",
    Default = false,
})

Toggles.SurvivalNoDrown:OnChanged(setWaterProtectionEnabled)
Toggles.SurvivalNoLava:OnChanged(setLavaProtectionEnabled)

local RenderingBox = Tabs.Mining:AddRightGroupbox("Rendering", "eye")

RenderingBox:AddLabel("Keeps ores from being hidden by ObjectRenderer.")

RenderingBox:AddToggle("ForceOreRender", {
    Text = "Force ore render",
    Default = false,
})

Toggles.ForceOreRender:OnChanged(setRenderBoostEnabled)
...
local FishingBox = Tabs.Mining:AddLeftGroupbox("Fishing", "fish")
...
cleanup = function()
    ...
    stopFishingHover()
    if EnvironmentState.NoDrown then
        setWaterProtectionEnabled(false)
    end

    if EnvironmentState.NoLava then
        setLavaProtectionEnabled(false)
    end

    if EnvironmentState.ForceOreRender then
        setRenderBoostEnabled(false)
    end
end
end)
